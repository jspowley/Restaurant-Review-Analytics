...
```{r, scraping_functions, echo = FALSE, message = FALSE, eval = run_scraping}
# credit to you Phillipe for the idea. Slightly rewordked into a way which makes sense to me.
scrape.sleep <- function(lowest = 4, highest = 10) {
  delay <- runif(1, min = lowest, max = highest)
  Sys.sleep(delay)
}

short.sleep <- function(){
  scrape.sleep(0.2,0.8)
}

tiny.sleep <- function(){
  scrape.sleep(0.05,0.2)
}
```

```{r, init, echo = FALSE, message = FALSE, eval = run_scraping}
# Manually configuring links comes with the benefit that we can manually check how far to iterate through a sequence of pages without overshooting. That way, any scripts on Yelps end which are specifically on the lookout for that kind of behavior won't see persistent 404 errors, preventing bans. In addition, the large volume of data accessible thorough these pages justifies the extra effort.

# Potential to automate later

# config <- readRDS("project_config.rds")

config <- data.frame(max_iter = c(230,230,230),
                      location = c("Victoria", "Edmonton", "Toronto"),
                      link = c("https://www.yelp.com/search?find_desc=Restaurants&find_loc=Victoria%2C+British+Columbia%2C+Canada","https://www.yelp.com/search?find_desc=Restaurants&find_loc=Edmonton%2C+AB%2C+Canada", "https://www.yelp.com/search?find_desc=Restaurants&find_loc=Toronto"))
```

```{r, firefox_pass1, echo = FALSE, message = FALSE, eval = run_scraping}
# Load firefox
rD <- RSelenium::rsDriver(browser = "firefox", chromever = NULL, verbose = FALSE)
remDr <- rD[["client"]]
Sys.sleep(2)

# Navigation logic,  pulling DOMs.
#nrow(config)
for(i in 1:nrow(config)){
  remDr$navigate(config$link[i])
  page_list <- c(remDr$getPageSource())
    
  scrape.sleep()
    
  j=10
    
  while(j<=config$max_iter[i]){
    
    link_in <- paste0(config$link[i], "&start=", j)
    remDr$navigate(link_in)
    page_in <- remDr$getPageSource()
    page_list <- append(page_list, page_in)

    scrape.sleep()
    j <- j + 10
  }
  pages <- list(page_list)
  saveRDS(pages, file = paste0(config$location[i],".rds"))
}
...
```{r, rest_tiles, echo = FALSE, message = FALSE, eval = run_scraping}

# Strips selected information from a restaurant tile panel, returning a single row data frame.

rest_tile_strip <- function(frame_in, parent){
  
  name <- frame_in %>% html_node("a.css-19v1rkv") %>% html_text()
  url <- frame_in %>% html_node("a.css-19v1rkv") %>% html_attr("href") %>%
    paste0(parent,.)
  rating_raw <- frame_in %>% html_node("span.css-gutk1c") %>% html_text()
  review_count_raw <- frame_in %>% html_node("span.css-chan6m") %>% html_text()

  tags <- frame_in %>% html_nodes("span.css-11bijt4") %>% html_text() %>% toString()
  
  data_out <- data.frame(name = name, 
                         link = url, 
                         rating = rating_raw, 
...
```{r, loading, echo = FALSE, message = FALSE, eval = run_scraping}

for(i in 1:nrow(config)) {
  
  city_pages <- readRDS(file = paste0(config$location[i],".rds"))
  rest_table <- data.frame(name = NA, link = NA, rating = NA, review_count = NA, tags = NA)
  
  for(subpage in city_pages[[1]]){
    
    elements <- read_html(subpage) %>%
    html_nodes(xpath = "//div[contains(@class, 'css-33bb9k') and not(contains(@class, 'ABP'))]")
  
    for(elem in elements){
      df.row <- elem %>% rest_tile_strip(parent = "https://www.yelp.com")
      rest_table <- rest_table %>% rbind(df.row)
    }
  }
  ...
  dplyr::mutate(rating = as.numeric(rating),
                review_count = str_extract(review_count, pattern = "[0-9]+") %>% as.numeric())
  
  saveRDS(rest_table, file = paste0(config$location[i],"_preprocessed.rds"))
}
```

```{r, rest_page_general, echo = FALSE, message = FALSE, eval = run_scraping}
rest_page_general <- function(page_in, ref_link){
  page_in <- page_in[[1]] %>% read_html()
  
  hours <- page_in %>% 
    html_nodes("table.hours-table__09f24__KR8wh.css-n604h6") %>% 
    html_nodes("tr") %>% 
    html_text()
  
  info_panel <- page_in %>% 
    html_nodes("div.css-s81j3n")
  
  address <- NA
  address.seg <- info_panel %>% 
    html_nodes("div.css-djo2w")
  for(a in address.seg){
    if(a %>% html_text() %>% str_detect(pattern = "Get Directions")){
      address <- a %>% html_text()
    }
  }
    
  poten_sites <- info_panel %>% 
    html_nodes("a.css-1idmmu3") %>% 
    html_attr("href")
  site_out <- NA
  for(site in poten_sites){
    if(!str_detect(site, pattern = "/map/")){
      site_out <- site
    }
  }
  
  buttons <- page_in %>% 
    html_nodes("a.css-4v7jbl")
  menu_link <- NA
  for(b in buttons){
    if(b %>% html_text() %>% str_detect(pattern = "Website menu")){
      menu_link <- b %>% html_attr("href")
    }
  }
  
  sections <- page_in %>% html_nodes("section.css-ufd2i")
  highlights_out <- c()
  for(s in sections){
    if(s %>% html_text() %>% str_detect(pattern = "Highlights from the Business")){
      highlights <- s %>% html_nodes("p.css-sihfu1")
      for(h in highlights) {
        highlights_out <- highlights_out %>% append(h %>% html_text())
      }
    }
  }
  
  price_score <- NA
  price_scores_in <- page_in %>% html_nodes("span.css-14r9eb") %>% html_text()
  for(e in price_scores_in){
    if(e %>% str_detect(pattern = "$")){
      price_score <- e
    }
  }
  
  photo_count <- page_in %>% html_nodes("a.css-1wpxijb") %>% html_text()
  
  attribute_list <- page_in %>% 
    html_nodes("div.arrange__09f24__LDfbs.gutter-2__09f24__CCmUo.layout-wrap__09f24__GEBlv.layout-2-units__09f24__PsGVW.css-1qn0b6x") %>% 
    html_nodes("div.arrange-unit__09f24__rqHTg.arrange-unit-fill__09f24__CUubG.css-1qn0b6x")
  attributes_out <- attribute_list %>% html_text()
  
  health_score_link <- NA
  for (a in attribute_list){
    if(a %>% html_text() %>% str_detect(pattern = "Health Score")){
      health_score_link <- a %>% 
        html_nodes("a.css-1idmmu3") %>% 
        html_attr("href")
    }
  ...
  popular_items <- NA
  popular_items <- page_in %>% 
    html_nodes(xpath = "//div[contains(@data-testid, 'scroll-container') and contains(@class, 'css-wtz7x')]") %>%
    html_nodes("img") %>% 
    html_attr("alt")
...
#  print(price_score)
#  print(photo_count)
#  print(attributes_out)
#  print(health_score_link)
  
#  out_temp <- list(hours = list(hours),
              #address = address,
              #restuarant_site = site_out,
              #menu_link = menu_link,
              #business_highlights = list(highlights_out),
              #price_score = price_score,
              #photo_count = photo_count,
              #attributes = list(attributes_out),
              #health_link = health_score_link)
  ...
  out <- data.frame(
              ...
              hours = I(list(hours)),
              business_highlights = I(list(highlights_out)),
              attributes = I(list(attributes_out)),
              popular_items = I(list(popular_items))
              )
  return(out)
}
```

```{r, stripping_rest_page, echo = FALSE, message = FALSE, eval = run_scraping, error = FALSE}

for(index in 1:nrow(config)){
city_df <- readRDS(file = paste0(config$location[index],"_preprocessed.rds"))
...
for(i in 1:nrow(city_df)){
  ...
  remDr$navigate(city_df$link[i])
  scrape.sleep()
  
  button <- tryCatch(
    remDr$findElement(using = "css", value = "button.css-r3lhny[aria-controls='expander-link-content-:Rjlecr:']"),
  ...
  # Feature Extraction
  general <- rest_page_general(page, city_df$link[i])
  if(i == 1){
    general_out <- general
  }else{
    general_out <- general_out %>% add_row(general)
  }
}

remDr$close()
rD[["server"]]$stop()
saveRDS(general_out, file = paste0(config$location[index],"_Rest_General.rds"))
}
```

```{r, review_function, echo = FALSE, message = FALSE, eval = run_scraping}
get_passport_stat <- function(review_in, aria_label){
  
  out <- NA
  
  p_items <- review_in %>% html_nodes("div.user-passport-stats__09f24__NQxB4.css-1qn0b6x") %>% html_nodes("div.css-volmcs")
  
  for(p in p_items){
    if(p %>% html_attr("aria-label") == aria_label){
      out <- p %>% html_text()
    }
  }
...
strip_reviews <- function(page, ref_link){
  
  reviews <- page[[1]] %>% read_html() %>% html_nodes(xpath = "//ul[contains(@class,'list__09f24__ynIEd')][li[div]]") %>% 
    html_nodes("li")
  
  reviews %>% length()
  review_out <- data.frame(text = NA, follow_up = NA, stars = NA, prior_stars = NA, prior_review_text = NA, prior_review_date = NA, review_date = NA, writer_origin = NA, images_posted = NA, user_id = NA, user_friends = NA, user_reviews = NA, user_photos = NA)
  i <- 1
  for(r in reviews){
    # print(r %>% html_text())
    # print(paste0("Count is ",i))
    # i <- i + 1
    
    review_out <- review_out %>% 
      add_row(text = r %>% html_node("p.comment__09f24__D0cxf.css-qgunke") %>% html_text(),
              
              follow_up = case_when(r %>% html_nodes("p.comment__09f24__D0cxf.css-qgunke") %>% length > 1 ~ TRUE, TRUE ~ FALSE),
              
              stars = r %>% html_node("span.css-1d8srnw") %>% html_node("div.css-14g69b3") %>% html_attr("aria-label"),
              
              review_date = r %>% html_node("div.css-10n911v") %>%
                html_node("div.arrange-unit__09f24__rqHTg.arrange-unit-fill__09f24__CUubG.css-1qn0b6x") %>% 
                html_node("span.css-chan6m") %>% html_text(),
              
              prior_review_date = case_when(follow_up ~ r %>% html_node("div.block-quote__09f24__qASfJ.css-kjl932") %>%
                                              html_node("div.arrange-unit__09f24__rqHTg.arrange-unit-fill__09f24__CUubG.css-1qn0b6x") %>%
                                              html_node("span.css-chan6m") %>% html_text(), TRUE ~ NA),
              
              prior_review_text = case_when(follow_up ~ r %>% html_node("div.block-quote__09f24__qASfJ.css-kjl932") %>% 
                                              html_node("span.raw__09f24__T4Ezm") %>% html_text(), TRUE ~ NA),
              
              prior_stars = case_when(follow_up ~ r %>% html_node("div.block-quote__09f24__qASfJ.css-kjl932") %>% 
                                        html_node("span.css-1d8srnw") %>% html_node("div.css-14g69b3") %>% html_attr("aria-label"), 
                                      TRUE ~ NA),
              writer_origin = r %>% html_node("span.css-qgunke") %>% html_text(),
              images_posted = r %>% html_nodes("img.css-xlzvdl") %>% length(),
              user_id = r %>% html_node("a.css-19v1rkv") %>% html_attr("href"),
              user_friends = r %>% get_passport_stat("Friends"),
              user_reviews = r %>% get_passport_stat("Reviews"),
              user_photos = r %>% get_passport_stat("Photos"))
  }
  review_out <- review_out %>% tail(-1)
  return(review_out)
}
```

```{r, test_page, echo = FALSE, message = FALSE, eval = run_scraping}
rD <- RSelenium::rsDriver(browser = "firefox", chromever = NULL, verbose = FALSE)
remDr <- rD[["client"]]

remDr$navigate("https://www.yelp.com/biz/blowers-and-grafton-edmonton-2?osq=Restaurants")
scrape.sleep()
page <- remDr$getPageSource()
...
#rating test
li_nodes <- page[[1]] %>% read_html() %>% html_nodes(xpath = "//ul[contains(@class,'list__09f24__ynIEd')][li[div]]") %>% 
  html_nodes("li")

li_nodes[10] %>% get_passport_stat("Photos")
li_nodes[10] %>% html_nodes("div.user-passport-stats__09f24__NQxB4.css-1qn0b6x") %>% html_nodes("div.css-volmcs")
```

```{r, review_extraction, echo = FALSE, message = FALSE, eval = run_scraping}
for(index in 1:nrow(config)){
city_df <- readRDS(file = paste0(config$location[index],"_preprocessed.rds")) %>% 
                     dplyr::mutate(review_count = case_when(is.na(review_count) ~ 0,
                                                            TRUE ~ review_count))
...
for(i in 1:nrow(city_df)){
  ...
  if(i == 1){
    city_reviews_out <- data.frame(ref_link = NA, review_df = NA)
  }
  max_iter <- floor(city_df$review_count[i]/10)
  if(floor(city_df$review_count[i]/10) == city_df$review_count[i]/10){
    max_iter <- max_iter - 1
  }
  
  root_link <- city_df$link[i]
  
  for(j in 0:max_iter){
    if(j == 0){
      remDr$navigate(root_link)
      scrape.sleep()
      page <- remDr$getPageSource()
      reviews_out <- strip_reviews(page)
    } else {
      remDr$navigate(paste0(root_link,"&start=",j*10))
      scrape.sleep()
      page <- remDr$getPageSource()
      reviews_out <- rbind(reviews_out, strip_reviews(page))
    }
  }
  
  city_reviews_out <- city_reviews_out %>% add_row(ref_link = city_df$link[i], review_df = nest(reviews_out))
  saveRDS(city_reviews_out, file = paste0(config$location[index],"_reviews.rds"))
}
remDr$close()
rD[["server"]]$stop()
}
...
```{r, merge_layers, echo = FALSE, message = FALSE, eval = run_scraping}
for(loc in config$location){
  rev <- readRDS(paste0(loc,"_reviews.rds")) %>% dplyr::rename(link = ref_link)
  gen <- readRDS(paste0(loc,"_Rest_General.rds"))
  master <- readRDS(paste0(loc,"_preprocessed.rds"))

  master %>% left_join(gen, by = "link") %>% left_join(rev, by = "link") %>% 
    saveRDS(file = paste0(loc,"_master.rds"))
}

for(i in 1:nrow(config)){
  df_in <- readRDS(paste0(config$location[i],"_master.rds")) %>% 
    dplyr::mutate(location = config$location[i])
  if(i == 1){
    master_df <- df_in
  } else {
    master_df <- master_df %>% rbind(df_in)
  }
}

master_df %>% unnest(review_df) %>% unnest(data) %>% saveRDS(file = "master_df.rds")
```
